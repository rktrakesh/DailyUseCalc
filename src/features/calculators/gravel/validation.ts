...
export function validateGravelInput(input: GravelInput): ValidationIssue[] {
  ...
  if (!isPositiveNumber(input.length.value))
    issues.push({ field: 'length', message: 'Enter a length greater than zero.' });
  if (!isPositiveNumber(input.width.value))
    issues.push({ field: 'width', message: 'Enter a width greater than zero.' });
  if (!isPositiveNumber(input.depth.value))
    issues.push({ field: 'depth', message: 'Enter a depth greater than zero.' });
  if (
  ...
  if (input.gravelType === 'custom' && !isPositiveNumber(input.customDensityTonsPerYard ?? 0)) {
    issues.push({
      ...
      message: 'Enter a custom density greater than zero.',
    });
  }
  if (
    input.pricePerCubicYard !== undefined &&
    (!Number.isFinite(input.pricePerCubicYard) || input.pricePerCubicYard < 0)
  ) {
    issues.push({ field: 'pricePerCubicYard', message: 'Price cannot be negative.' });
  }
  if (
    input.deliveryFee !== undefined &&
    (!Number.isFinite(input.deliveryFee) || input.deliveryFee < 0)
  ) {
  ...
  if (
    ...
    issues.push({
      ...
      message: 'Truck capacity must be greater than zero.',
    });
  }
  return issues;
}
