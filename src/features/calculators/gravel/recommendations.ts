...
const DEPTH_GUIDANCE: Record<ProjectType, string> = {
  driveway:
    'Most residential driveways need a compacted gravel layer of about 4–6 inches; local soil and vehicle loads can require more.',
  ...
  'french-drain':
    'French drains need enough depth to surround the pipe and support drainage; confirm the trench design for your property.',
  'shed-base':
    'A shed base commonly uses 4–6 inches of compacted gravel on a level, prepared area.',
...
const MATERIAL_GUIDANCE: Record<GravelType, string> = {
  'crushed-stone':
    'Crushed stone compacts well and is a common choice for driveways and structural bases.',
  'pea-gravel':
    'Pea gravel is smooth and decorative, but its rounded shape does not lock together like crushed stone.',
  'river-rock':
    'River rock is often used for decorative beds and drainage, where a rounded finish is preferred.',
  limestone: 'Limestone compacts well and is commonly used as a durable base material.',
  granite:
    'Granite gravel is durable and can be suitable where a hard-wearing aggregate is needed.',
  custom:
    'Use a supplier-confirmed density for custom material so the weight estimate stays useful.',
};

export function recommendGravel(
  input: GravelInput,
  ...
  if (input.allowancePercent === 0) {
    warnings.push(
      'No allowance is included. A small allowance can help cover uneven ground, compaction, and handling loss.',
    );
  }
  if (input.gravelType === 'pea-gravel' && input.projectType === 'driveway') {
    warnings.push(
      'Pea gravel can shift under vehicle traffic. Consider a material that compacts and interlocks for a driveway surface.',
    );
  }
  return {
